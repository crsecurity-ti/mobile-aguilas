...
import { prettifyRut, removeSeparators, calculateDv } from "../utils/rutUtils";
...
export const useRut = () => {
  ...
  const updateRut = (rut: string, callback?: (rut: string) => void) => {
    setRut(removeSeparators(rut));
    callback?.(prettifyRut(rut));
  };
  ...
  const isValidRut = (rut: string) =>
    rut !== "" &&
    rut?.slice(0, -1).replace(/\./g, "") !== "0-" &&
    dv === calculateDv(parseInt(rut?.slice(0, -1).replace(/\./g, "")));
