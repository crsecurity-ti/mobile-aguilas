...
import useHourManagement from "../../../../hooks/useHourManagement";
...
const HourManagementCard = () => {
  ...
  const { hourManagementData, loading: hourManagementLoading } =
    useHourManagement({
      contractorUuid: user?.userInformation.contractorUuid as string,
      userUuid: user?.uuid as string,
    });

  const { contractorData, loading: contractorLoading } = useContractor({
    contractorUuid: user?.userInformation.contractorUuid as string,
  });
  ...
  const endHour = async () => {
    await updateHourManagementInFirestore({
      contractorUuid: user?.userInformation.contractorUuid as string,
      hourManagementUuid: hourManagementData?.find((hourManagement) =>
        isAfter(
          addMinutes(
